...
describe("GET /categories", () => {
  ...
  it("Should return categories with v in in the category name", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.data.every((item: { name: string }) => item.name.includes("v"))).toBe(true)
...
describe("POST /categories", () => {
  ...
  const storeCategory = () => request().post("/api/categories").set("Cookie", `accessToken=${adminAccessToken}`)
  ...
  it("Should return 201 with the category", async () => {
    ...
    expect(await db.category.findUnique({
      where: {
        name: category.name
      }
    })).toBeTruthy()
...
describe("UPDATE /categories/:id", () => {
  ...
  const updateCategory = (id: number) => request().patch(`/api/categories/${id}`).set("Cookie", `accessToken=${adminAccessToken}`);
  ...
  it("Should return 401 with category already exists", async () => {
    ...
    const response = await updateCategory(category.id).send({ name: category.name })
...
describe("DELETE /categories/:id", () => {
  ...
  const deleteCategory = (id: number) => request().delete(`/api/categories/${id}`).set("Cookie", `accessToken=${adminAccessToken}`)
...
describe("AUTHORIZATION", () => {
  ...
  it("Should return 401 when user is not admin", async () => {
    ...
    let response = await request().post("/api/categories").set("Cookie", `accessToken=${userAccessToken}`);
    expect(response.status).toBe(401)

    response = await request().patch("/api/categories/xxx").set("Cookie", `accessToken=${userAccessToken}`);
    expect(response.status).toBe(401)

    response = await request().delete("/api/categories/xxx").set("Cookie", `accessToken=${userAccessToken}`);
    expect(response.status).toBe(401)
