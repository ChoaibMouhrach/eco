...
const validate: Validate = (request: AuthRequest) => {
  const schema = z.object({
    name: z.string().min(1).max(255).refine(async (name) => !(await db.tag.findUnique({ where: { name } })), { message: "Tag already exists" }),
    xId: z.string().regex(/^\d+$/ig).pipe(z.string().transform(v => Number(v)).refine(async (id) => await db.tag.findUnique({ where: { id } }), { message: "Tag not found" }))
  })
