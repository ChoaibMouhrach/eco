...
const validate: Validate = (request: Request) => {
  const schema = z.object({
    ...
    email: z.string().email().refine(async (email) => !(await db.user.findUnique({ where: { email } })), { message: "Email Address already exists" }),
    phone: z.string()
      .regex(/^\+[1-9]\d{1,14}$/)
      .refine(async (phone) => !(await db.user.findUnique({ where: { phone } })), { message: "Phone is already taken" }),
    address: z.string().min(3).max(255),
    roleId: z.number().refine(async (id) => await db.role.findUnique({ where: { id } }), { message: "Role not found" })
  });
