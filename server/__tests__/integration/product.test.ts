...
import { categoryPayload, productPayload, userPayload } from "../../src/common/constants/testData.constant";
...
describe("POST /products", () => {
  ...
  it("Should return 400 with At least one category is required", async () => {
    ...
    expect(response.status).toBe(400)
    expect(response.body?.statusCode).toBe(400)
    expect(response.body?.message).toMatchObject([{ message: "At least one category is required", path: ["categories"] }])
    expect(response.body?.error).toBe("Bad Request")
  ...
  it("Should return 400 Images are required", async () => {
    ...
    expect(response.status).toBe(400)
    expect(response.body?.statusCode).toBe(400)
    expect(response.body?.message).toMatchObject([{ message: "Required", path: ["images"] }])
    expect(response.body?.error).toBe("Bad Request")
